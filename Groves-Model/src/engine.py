...
def build_qpl_fact(wb, pl_csv_path, config):
    ...
    for month in months:
        ...
        raw[(month, 'Cash-on-Cash Return')] = (cfads * 12) / config['total_equity'] if config['total_equity'] != 0 else 0
        raw[(month, 'Monthly Cash-on-Cash (Ann.)')] = (cfads * 12) / config['total_equity'] if config['total_equity'] != 0 else 0
